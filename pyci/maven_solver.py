...
if 1 == len(sys.argv):
    ...
    --exclude=cloud-business-components/common-business
    --exclude=cloud-business-components/cloudportal-relate/portal-cms-static
    --include=cloud-basic-components
    --include=cloud-business-components
    --include=cloud-technical-components
    --output=sequence
    '''
    cmd = u'''
    --target=com.gsafety.cloudframework:cloudframework
    --root=E:/svn/CFWRefactor
    --output=sequence
    --exclude=cloud-business-components/cloud-portal-components/portal-cms-static
    '''
    
    '''
    --exclude=cloud-business-components/cloud-common-components
    '''
...
print('parsing poms')
poms = [MavenUtils.Pom.from_pom(path) for path in paths]

print('graph tree')
MavenUtils.tree(poms=poms, target='maven_parent_new.gml')

# print('sequenceing %s' % args['target'])
# poms = MavenUtils.sequence(poms=poms, target=args['target'], log=False)
...
#     count += 1
#     print(count)
#     if not ProcessUtils.process(MavenUtils.maven_build(pom.path)): break